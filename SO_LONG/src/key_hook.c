...
void	ft_where_is(t_game *g)
{
	...
	if (g->map[g->player_ps[1]][g->player_ps[0]] == 'C')
	{
        ...
        while (i < counter)
        {
            if((g->collect_img->instances[i].y / S == g->player_ps[1])
               && (g->collect_img->instances[i].x / S == g->player_ps[0])
...
void   ft_delete(t_game *g)
{
    ...
    while (i < counter)
    {
        if((g->collect_img->instances[i].y /S == g->player_img->instances->y)
            && (g->collect_img->instances[i].x /S == g->player_img->instances->x)
            && (g->collect_img->instances[i].enabled == true))
        {
            counter --;
            g->collect_img->instances[i].enabled = false;
